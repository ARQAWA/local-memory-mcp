...
For any task that needs discovery, planning, edits, tests, review, or more than one meaningful step, open a Task Working Memory workbench with open_task_memory after initial recall. Keep it updated with update_task_memory while you discover code by layers, analyze, design, reject options, implement, test, review, and track risks. Use layered discovery for routes/endpoints, services, repositories, clients, permissions/auth, configs, tests, data contracts, docs, active install, and runtime when relevant. Before reporting done, close it with close_task_memory so durable learnings are digested and short-lived scratch is removed or explicitly retained.
